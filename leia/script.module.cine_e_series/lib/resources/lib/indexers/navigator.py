...
class navigator:
    ...
    LOCALNEWS     = os.path.join(THISADDONPATH, 'cine_e_series Updates.txt')
    
    def root(self):
        self.addDirectoryItem('[COLOR=gold]cine_e_series Updates[/COLOR]', 'newsNavigator', 'tools.png', 'DefaultAddonProgram.png')
    ...
    def news(self):
            ...
            if len(message)>1:
                    if compfile == message:pass
                            ...
                            text_file.close()
                            compfile = message
            self.showText('[B][I][COLOR yellow]cine_e_series Latest Updates[/COLOR][/B][/I]', compfile)
