...
def cache_version_check():
    if _find_cache_version():
        cache_clear()
        cache_clear_providers()
        #control.execute('RunPlugin(plugin://%s)' % 'plugin.video.cine_e_series/?action=cleanSettings')
...
def _find_cache_version():
    ...
    try:
        curVersion = control.addon('plugin.video.cine_e_series').getAddonInfo('version')
