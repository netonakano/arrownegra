...
class seasons:
    ...
    def tmdb_list(self, tvshowtitle, year, imdb, tmdb, meta=None, lite=False):
        ...
        try:
            ...
            item = client.scrapePage(seasons_url).json()
            if item == None:
                raise Exception()
    ...
    def seasonDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                stats =  re.findall('(\d{4})', i['premiered'])[0] or i['premiered']
...
class episodes:
        ...
        def items_list(i):
            ...
            try:
                ...
                if item.get('status_code') == 34:
                    ...
                    item = client.scrapePage(url2).json()
                try:
    ...
    def fanart_tv_art(self, tvdb):
        ...
        try:
            ...
            art = client.scrapePage(self.fanart_tv_art_link % tvdb, headers=fanart_tv_headers).json()
        except:
            artmeta = False
        if artmeta == False:
            pass
    ...
    def tmdb_list(self, tvshowtitle, year, imdb, tmdb, season, meta=None, lite=False):
        ...
        try:
            ...
            result = client.scrapePage(episodes_url).json()
            ...
            return
        try:
            poster = self.tmdb_image_link + result['poster_path']
        except:
            poster = ''
        if not poster:
            poster = '0'
        ...
        for item in episodes:
            try:
                ...
                if not episodeplot:
                    ...
                    episodeplot = client.replaceHTMLCodes(six.ensure_str(episodeplot, errors='replace'))
                try:
                    r_crew = item['crew']
                    director = [d for d in r_crew if d['job'] == 'Director']
                    director = ', '.join([d['name'] for d in director])
