...
import urllib.parse as urlparse, urllib.request as urllib2, subprocess, os,traceback,cookielib,re,Queue,threading
...
def getUrl(url,timeout=30,returnres=False,stream=False):
    global cookieJar
    global clientHeader
    try:
        post=None
...
def getUrlold(url,timeout=35, returnres=False):
    global cookieJar
    global clientHeader
    try:
        post=None
...
def download_chunks(URL, chunk_size=4096, enc=None):
    ...
    conn=getUrl(URL,timeout=18,returnres=True,stream=True)
    #while 1:
    if enc:
        if USEDec==1 :
            chunk_size*=1000
