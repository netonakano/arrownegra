...
def getRequest3(url,ref,userargent=False):
    try:
        import urllib.request as urllib2
    except ImportError:
        import urllib2
    ...
    if match1 !=[]:
        ...
        for other,channel_name,stream_url in match2:
            ...
            if cat > '':
                ...
                elif f4mtester and not '.mp4' in stream_url and not '.mkv' in stream_url and not '.avi' in stream_url and not '.rmvb' in stream_url and not 'pluto.tv' in stream_url and not 'plugin' in stream_url:
