...
class MyHandler(BaseHTTPRequestHandler):
   ...
   """
    ...
    def answer_request(self, sendData):
        ...
        try:
            ...
            if request_path.lower()=="sendvideopart":
                ...
                return
            initDone=False
            (url,proxy,use_proxy_for_chunks,maxbitrate,simpledownloader, auth,streamtype,swf ,callbackpath, callbackparam, referer, origin, cookie)=self.decode_url(request_path)
    ...
    #                crange="bytes "+str(srange)+"-" +str(int(totalsize-1))+"/"+str(totalsize)#recalculate crange based on srange, portionLen and content_size 
                ...
                if not downloader.init(self.wfile,url,proxy,use_proxy_for_chunks,g_stopEvent,maxbitrate,auth , callbackpath, callbackparam,referer,origin,cookie):
                    #print 'cannot init'
            ...
            #g_stopEvent.set()
            if not initDone:
                #xbmc.executebuiltin("Notification(F4mProxy,%s,4000,'')"%inst.message)
                xbmc.executebuiltin("Notification(F4mProxy,%s,4000,'')"%str(inst))
                self.send_error(404)
            ##print 'sending 404'
    ...
    def decode_url(self, url):
        ...
        return (received_url,proxy,use_proxy_for_chunks,maxbitrate,simpledownloader,auth,streamtype,swf ,callbackpath, callbackparam, referer, origin, cookie )   
    """
...
class f4mProxy():
    ...
    def prepare_url(self,url,proxy=None, use_proxy_for_chunks=True,port=PORT_NUMBER, maxbitrate=0,simpleDownloader=False,auth=None, streamtype='HDS',swf=None, callbackpath="",callbackparam="", referer="", origin="", cookie=""):
        global PORT_NUMBER
        global PORT_NUMBER
        newurl=urllib.urlencode({'url': url,'proxy':proxy,'use_proxy_for_chunks':use_proxy_for_chunks,'maxbitrate':maxbitrate,'simpledownloader':simpleDownloader,'auth':auth,'streamtype':streamtype,'swf':swf,'callbackpath':callbackpath , 'callbackparam':callbackparam, 'referer':referer, 'origin':origin,'cookie':cookie})
        link = 'http://'+HOST_NAME+(':%s/'%str(port)) + newurl
        return (link) #make a url that caller then call load into player

class f4mProxyHelper():

    def playF4mLink(self,url,name,proxy=None,use_proxy_for_chunks=False, maxbitrate=0, simpleDownloader=False, auth=None, streamtype='HDS',setResolved=False,swf=None , callbackpath="",callbackparam="", referer="", origin="", cookie="", iconImage="DefaultVideo.png"):
        try:
            #print "URL: " + url
            ...
            progress.create('F4mProxy','Conectando...')
            stream_delay = 1
    ...
    def start_proxy(self,url,name,proxy=None,use_proxy_for_chunks=False, maxbitrate=0,simpleDownloader=False,auth=None,streamtype='HDS',swf=None, callbackpath="",callbackparam="",referer="",origin="",cookie=""):
        #print "URL: " + url
        stopPlaying=threading.Event()
        f4m_proxy=f4mProxy()
        stopPlaying.clear()
